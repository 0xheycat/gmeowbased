# Phase 1D: Frame UI/Layout Enhancement & Data Audit - Planning Document

**Status**: Planning  
**Created**: 2025-01-28  
**Updated**: 2025-11-23 (Major Revision)  
**Target Completion**: Week of December 1, 2025  
**Dependencies**: Phase 1C completion (Tasks 1-6) ✅

---

## 🎯 Phase 1D Mission Change

**Original Plan**: Analytics, A/B testing, personalization (deferred to Phase 1E)  
**Revised Plan**: **UI/Layout improvements + comprehensive data audit**

**Why the Change**:
- Mock data removed in commit `b9c2ce9` - need to validate real data displays correctly
- Frame visual design needs polish before analytics (garbage in = garbage out)
- User feedback indicates layout/clarity issues in several frame types
- Foundation must be solid before building analytics layer

---

## Executive Summary

Phase 1D focuses on **UI/layout refinement** and **data consistency validation** across all 9 Farcaster frame types. After replacing mock data with real Supabase queries, we must ensure frames display user data accurately, clearly, and beautifully. This phase polishes visual design, validates data integrity, and enhances the rich embed experience.


**Core Objectives**:
1. **UI/Layout Audit** - Review all 9 frame types for visual clarity and consistency
2. **Data Validation** - Verify real Supabase data displays correctly (post mock data removal)
3. **Rich Embed Enhancement** - Improve metadata, images, and user experience
4. **Frame-to-App Consistency** - Ensure frame data matches dashboard/app displays
5. **Error Handling** - Add graceful fallbacks for missing data, API failures

---

## 🎨 The 9 Frame Types - Current Status

### Frame Type Overview

| # | Frame Type | Purpose | Data Source | UI Status | Data Status |
|---|------------|---------|-------------|-----------|-------------|
| 1 | **Quest** | Display quest details, launch miniapp | Supabase quests table | 🟡 Needs polish | ✅ Real data |
| 2 | **Guild** | Show guild info | ⚠️ NO TABLE (mock data) | 🟡 Basic layout | ❌ Placeholder |
| 3 | **Points** | User points balance | Supabase user_profiles | 🟡 Username fix done | ✅ Real data |
| 4 | **Referral** | Referral stats | ⚠️ NO TABLE (mock data) | 🟡 Basic layout | ❌ Placeholder |
| 5 | **Leaderboard** | Top players | Supabase leaderboard_snapshots | 🟢 Complex layout | ✅ Real data |
| 6 | **GM** | Daily GM ritual | Supabase gmeow_rank_events | 🟡 Needs streak display | ✅ Real data (commit b9c2ce9) |
| 7 | **Badge** | Badge collection | Supabase user_badges, badge_templates | 🟡 Needs visual hierarchy | ✅ Real data (commit b9c2ce9) |
| 8 | **Verify** | Quest verification | Supabase + on-chain | 🟡 Basic layout | ✅ Real data |
| 9 | **OnchainStats** | On-chain metrics | RPC + Supabase | 🟡 Dense layout | ✅ Real data |

**Legend**:
- 🟢 **Good** - UI is clear, data displays well
- 🟡 **Needs Work** - Functional but could be improved
- 🔴 **Critical** - Broken or confusing
- ✅ **Real Data** - Using Supabase queries
- ❌ **Mock Data** - Placeholder until table created
...
**Commit**: `b9c2ce9` - "Replace mock data with real Supabase queries" (GM, rank, badges)
...
### Feature 1: UI/Layout Audit & Enhancement


**Objective**: Review and improve visual clarity, layout, and user experience across all frame types.

#### 1.1 GM Frame UI Improvements

**Current Issues**:
- ❌ GM count and streak display not prominent enough
- ❌ No visual indication of daily progress (e.g., "GM sent today ✅")
- ❌ Missing motivational copy for streak milestones (7-day, 30-day, etc.)

**Proposed Enhancements**:
```typescript
// GM Frame Title - show streak milestone
if (streak >= 30) {
  title = `🔥 ${streak}-Day Streak! Legendary GM Master`
} else if (streak >= 7) {
  title = `⚡ ${streak}-Day Streak! Keep it going!`
} else {
  title = `☀️ Good Morning! Start your streak today`
}

// Description - add daily status
const lastGMToday = checkIfGMSentToday(gmHistory)
description = lastGMToday 
  ? `✅ GM sent today! Streak: ${streak} days • Total GMs: ${gmCount}`
  : `☀️ Send your GM to continue your ${streak}-day streak!`
```

**Success Metrics**:
- Users can immediately see their current streak status
- Streak milestones are celebrated (7-day, 30-day badges)
- Call-to-action is clear ("Send GM Now" vs. "Streak Alive ✅")
...
**Current Issues**:
- ❌ Earned and eligible badges shown with equal emphasis
- ❌ No rarity indicators (common, rare, legendary)
- ❌ Badge images not displayed (only names)
