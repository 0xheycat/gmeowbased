      ...
      {totals ? (
        <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3">
          {METRIC_DEFINITIONS.map((metric) => {
            ...
            return (
              <div key={metric.key} className="rounded-xl border border-white/10 bg-white/5 px-3 py-3">
        ...
        <div className="mt-5">
          ...
          <div className="overflow-hidden rounded-xl border border-white/10">
            <table className="w-full text-left text-[11px] sm:text-[12px]">
              <thead className="bg-white/5 text-[var(--px-sub)]">
                <tr>
              ...
              <tbody>
                {chains.map((chain) => (
                  <tr key={chain.chain} className="odd:bg-white/5">
        ...
        {notes.length ? (
          <div className="flex flex-wrap items-center gap-2">
            {notes.slice(0, 3).map((note, idx) => (
              <span key={`${note}-${idx}`} className="pixel-pill bg-white/5 text-[10px]">
                {note}
              </span>
            ))}
          </div>
