...
export function SegmentedToggle<T extends string>({ options, value, onChange, size = 'md', ariaLabel }: SegmentedToggleProps<T>) {
	const toneClass: Record<SegmentedToggleTone, string> = {
		sky: 'border-sky-400/60 bg-gradient-to-r from-sky-400/80 via-blue-400/80 to-indigo-500/80 text-slate-900 shadow-lg shadow-sky-500/25',
		emerald: 'border-emerald-400/60 bg-gradient-to-r from-emerald-300/80 via-emerald-400/80 to-teal-400/80 text-slate-900 shadow-lg shadow-emerald-400/25',
		...
		neutral: 'border-white/40 bg-white/90 text-slate-900 shadow-lg shadow-white/10',
	}
		...
		<div className="flex flex-wrap gap-2" role="radiogroup" aria-label={ariaLabel}>
			{options.map((option, index) => {
				...
				const stateClass = isDisabled
					? 'cursor-not-allowed border border-white/12 text-slate-500/70 opacity-60'
					: active
						? toneClass[tone]
						: baseInactive
				// @edit-end
				return (
					...
					>
						...
						{option.badge ? (
							<span className={`w-fit rounded-full px-2 py-0.5 text-[10px] uppercase tracking-[0.18em] ${active ? 'bg-black/20 text-black' : 'bg-white/10 text-slate-300'}`}>
								{option.badge}
							</span>
						) : null}
					</button>
