      ...
      {error ? (
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr className="border-b border-white dark:border-slate-700/10 text-left text-[10px] uppercase tracking-wider text-slate-950 dark:text-white/60">
                ...
                >
                    ...
                    <div className="flex items-center gap-2">
                      <div className="relative h-8 w-8 flex-shrink-0 overflow-hidden rounded-full border border-white dark:border-slate-700/20">
                        {cast.avatar_url ? (
                          ...
                          <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-emerald-500/30 to-purple-500/30 text-[10px] font-bold text-slate-950 dark:text-white">
                            {cast.username?.[0]?.toUpperCase() ?? '?'}
                          </div>
                        )}
                      </div>
                      <div className="min-w-0">
                        <div className="truncate text-[12px] font-semibold text-slate-950 dark:text-white">
                          {cast.display_name ?? cast.username ?? `FID ${cast.fid}`}
                        </div>
                        {cast.username && (
                          <div className="truncate text-[10px] text-slate-950 dark:text-white/50">
                            @{cast.username}
                          </div>
                        )}
                      </div>
                  ...
                  <td className="py-3 pr-3">
                    <div className="flex flex-col items-end gap-0.5">
                      ...
                      <div className="flex gap-1.5 text-[9px] text-slate-950 dark:text-white/50">
                  ...
                  {/* Date */}
                  <td className="py-3">
                    <span className="text-[10px] text-slate-950 dark:text-white/50">
