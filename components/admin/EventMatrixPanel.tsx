...
export default function EventMatrixPanel() {
  ...
  return (
    <div className="space-y-6">
      <header className="rounded-3xl border border-white/10 bg-white/5 p-5 shadow-lg shadow-emerald-500/5 backdrop-blur">
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="min-w-[220px] max-w-xs">
            <label className="flex items-center gap-2 rounded-2xl border border-white/12 bg-white/10 px-3 py-2 text-[12px] text-white/70 focus-within:border-emerald-400/60 focus-within:text-white/90">
              <span className="text-[11px] uppercase tracking-[0.12em] text-white/60">Search</span>
              <input
                value={query}
      ...
      {grouped.length === 0 ? (
        <div className="rounded-3xl border border-white/10 bg-white/5 p-6 text-center text-[13px] text-white/70">
      ...
      {grouped.map((group) => (
        <section key={group.category} className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[11px] text-white/50">{group.events.length} event(s)</span>
          </div>
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
            {group.events.map((event) => (
              <article
                key={event.signature}
                className="group relative overflow-hidden rounded-3xl border border-white/12 bg-white/5 p-5 shadow-sm shadow-emerald-500/5 transition hover:border-emerald-300/50"
              >
                ...
                <div className="relative z-10 flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 break-all font-mono text-[11px] text-white/60">{event.signature}</p>
                  </div>
                ...
                <p className="relative z-10 mt-4 text-[13px] text-white/75">{event.description}</p>

                <div className="relative z-10 mt-4 rounded-2xl border border-white/10 bg-white/[0.04] p-4">
                  <div className="text-[11px] uppercase tracking-[0.14em] text-white/50">Inputs</div>
                  {event.inputs.length ? (
                    <ul className="mt-3 space-y-1 text-[12px] text-white/70">
                      {event.inputs.map((input, idx) => (
                        <li key={`${event.name}-input-${idx}`} className="flex flex-wrap items-center gap-2 font-mono">
                          ...
                          {input.indexed ? (
                            <span className="text-[10px] uppercase tracking-[0.14em] text-white/40">indexed</span>
                          ) : null}
                        </li>
                      ))}
                    </ul>
                  ) : (
                    <p className="mt-3 text-[12px] text-white/50">No indexed parameters.</p>
                  )}
                </div>
                ...
                {event.notification ? (
                  <div className="relative z-10 mt-4 rounded-2xl border border-amber-400/30 bg-amber-500/10 p-4">
                    ...
                    <p className="mt-3 text-sm font-semibold text-white">{event.notification.title}</p>
                    <pre className="mt-2 whitespace-pre-wrap rounded-2xl border border-white/10 bg-black/40 p-3 text-[12px] text-amber-100/90">{event.notification.body}</pre>
                    ...
                    {event.notification.preview ? (
                      <div className="mt-3 rounded-2xl border border-white/10 bg-white/5 p-3 text-[12px] text-white/80">
                        <span className="font-semibold text-white/90">Preview:</span> {event.notification.preview}
                      </div>
                    ) : null}
                  </div>
                ) : null}
                ...
                <div className="relative z-10 mt-4 border-t border-white/10 pt-4">
                  <div className="text-[11px] uppercase tracking-[0.14em] text-white/50">References</div>
                  <ul className="mt-2 space-y-1 text-[12px] text-white/60">
                    {event.references.map((ref) => (
                      <li key={`${event.name}-${ref.path}-${ref.symbol ?? 'ref'}`}>
                        <span className="font-semibold text-white/80">{ref.label}</span>
                        <span className="ml-2 text-white/50">{ref.path}</span>
                        {ref.symbol ? (
                          <span className="ml-1 text-white/40">({ref.symbol})</span>
                        ) : null}
                        {ref.note ? <span className="ml-1 text-white/50">— {ref.note}</span> : null}
                      </li>
                    ))}
                  </ul>
                </div>
