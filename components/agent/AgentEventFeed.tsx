...
export function AgentEventFeed({ events, emptyHint }: AgentEventFeedProps) {
  ...
  if (!items.length) {
    return (
      <EmptyState
        icon={<Sparkle className="size-8 text-white/60" weight="duotone" />}
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div className="space-y-2">
                  <div className="flex flex-wrap items-center gap-2 text-[11px] uppercase tracking-[0.22em] text-white/60">
                    ...
                    {chainLabel ? (
                      <span className="pixel-pill border-white/10 bg-white/5 text-[10px] text-white/70">{chainLabel}</span>
                    ) : null}
                    {isNew ? (
                  ...
                  <CardTitle asChild>
                    <h3 className="text-lg font-semibold text-white sm:text-xl">{event.headline}</h3>
                  </CardTitle>
                  <CardDescription className="text-sm text-white/80">
                    {event.context ?? 'Telemetry synced.'}
                    <span className="ml-2 text-xs text-white/60">{actorLabel}</span>
                  </CardDescription>
                  <CardFooter className="flex flex-wrap items-center gap-2 text-[11px] text-white/65">
                    <span className="inline-flex items-center gap-1">
                      <Clock className="size-3.5" weight="bold" />
                      {formatRelativeTime(event.createdAt)}
                    </span>
                    {event.delta != null ? (
                      <span className="pixel-pill border-white/10 bg-white/5 text-xs text-white/80">
                    ...
                    {tierLabel ? (
                      <span className="pixel-pill border-white/10 bg-white/5 text-xs text-white/80">{tierLabel}</span>
                    ) : null}
                  </CardFooter>
                </div>
                {event.cta ? (
                  <a
                    className="inline-flex items-center gap-2 self-start rounded-full border border-white/20 bg-white/5 px-3 py-1.5 text-xs font-semibold text-white/90 transition hover:border-emerald-200/60 hover:bg-emerald-400/10"
                    href={event.cta.href}
                  >
                    {event.cta.label}
