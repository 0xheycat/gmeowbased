      ...
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button className={`pixel-pill ${global ? 'bg-indigo-600 text-slate-900 dark:text-white' : ''}`} onClick={() => setGlobal(v => !v)}>
              {global ? 'Global' : 'Chain'}
            </button>
          </div>
        </div>
        ...
        <div className="flex gap-2 flex-wrap">
          {['all','follow','recast','reply','like','frame','onchain'].map(f => (
            <button key={f} className={`px-3 py-1 rounded-md text-sm ${filter===f ? 'bg-indigo-600 text-slate-900 dark:text-white' : 'bg-slate-100/90 dark:bg-white/5'}`} onClick={() => setFilter(f)}>{f === 'all' ? 'All Types' : f}</button>
          ))}
        </div>
                ...
                <motion.div key={r.address} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }} className={`relative pixel-card flex items-center justify-between gap-4`}>
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="mt-2 w-full bg-slate-100/5 dark:bg-white/5 h-2 rounded overflow-hidden"><div style={{ width: `${progress}%` }} className={`h-full bg-gradient-to-r from-indigo-400 to-emerald-400`} /></div>
                    </div>
                  </div>
