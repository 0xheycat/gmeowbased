/**
 * Dashboard - Trending Channels API
 * 
 * Fetches trending Farcaster channels (7d window)
 ...
 * 
 * GET /api/dashboard/trending-channels
 * Response: { data: TrendingChannel[], cached_at: string, ttl: number }
 */
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // ============================================================================
    // BUSINESS LOGIC: Fetch trending channels
    // ============================================================================
    const response = await getTrendingChannels()

    const duration = Date.now() - startTime
    console.log(`[Trending Channels API] Completed in ${duration}ms | Request-ID: ${requestId}`)
    ...
    console.error('[Trending Channels API] Error:', error)
    logApiRequest({
      method: 'GET',
      path: '/api/dashboard/trending-channels',
      status: 500,
